...
prompt = misc_classes.Text("...")
prompt = misc_classes.Text(f"\"Arise, {character_creation.u_name}, arise!\"")
...
prompt = misc_classes.Text(f"You are about to go forward and drink, but you see a person sitting on the bank on a boulder, swinging {character_creation.c_p_p.lower()} shoed foot against the face of the waters and splashing the water.")
...
while True:
	...
	match demo_stranger_choice:
		case "1":
			...
			prompt = misc_classes.Text(f"After a moment of consideration, you stagger over to the stranger. \"E-excuse me,\" you say, barely audible, and {character_creation.c_p.lower()} turns, turning {character_creation.c_eye_color} eyes upon you.")
			prompt = misc_classes.Text(f"\"Yes? Can I help you? You look quite haggard,\" {character_creation.c_p.lower()} hums, ceasing {character_creation.c_p_p.lower()} foot from swinging. {character_creation.c_p} brings it up to rest on the dry bank, gazing up at you expectantly.")
			...
			prompt = misc_classes.Text(f"The stranger blinks before giving a short sigh. \"That's obvious,\" {character_creation.c_p.lower()} mutters, rising to {character_creation.c_p_p.lower()} feet. {character_creation.c_p} hastens to your side and helps you hobble to the side of the river.")
			prompt = misc_classes.Text(f"{character_creation.c_p} lowers you gently to the water's edge and supports you while you take a refreshing drink.")
			prompt = misc_classes.Text(f"Relief and vigor surges through your body as you take another drink, and the stranger laughs. \"One would think you were starved from water for days if they saw you drinking like this,\" {character_creation.c_p.lower()} grins.")
			prompt = misc_classes.Text(f"Despite your slight annoyance at {character_creation.c_p_p.lower()} laughter and teasing about your horrible condition, you can't help but feel very grateful as you dip your stinging hands once more into the rushing water.")
			...
			prompt = misc_classes.Text(f"{character_creation.c_p} looks at you carefully, anaylzing your countenance before silently taking {character_creation.c_p_p.lower()} pouch from {character_creation.c_p_p.lower()} hip and unlacing the lid.")
			prompt = misc_classes.Text(f"\"Well, traveler, you're in luck today,\" {character_creation.c_p.lower()} muses, taking out a bandage and soaking it in the water. You glance at {character_creation.c_a_p.lower()}, watching {character_creation.c_p_p.lower()} hands wring out the cloth.")
			...
			prompt = misc_classes.Text(f"\"No need to thank me, friend. But not only that, had you gotten here any sooner, you would've been swarmed by a group of bandits. And I daresay, you are in no position of taking them out,\" {character_creation.c_p.lower()} hums.")
			prompt = misc_classes.Text(f"You pause, looking at {character_creation.c_a_p.lower()} in slight bewilderment. No traces...anywhere...? \"Oh...you took them out?\"")
			...
			prompt = misc_classes.Text(f"{character_creation.c_p} gives a slight smile, turning {character_creation.c_p_p.lower()} eyes to the sky. \"You don't know, hm? Well, you're in the world where nothing makes sense and the more you think of it, the less you know.\"")
			prompt = misc_classes.Text(f"{character_creation.c_p} leans towards you slightly, looking at you with a sly look in {character_creation.c_p_p.lower()} {character_creation.c_eye_color} orbs. \"A world where you are nothing but a pawn - nay, a dustspeck on the chessboard.\"")
			
			cinematic_swell = misc_classes.Sound_effect("cinematic_swell", 7, "cinematic_swell_channel", 0, 0, 0)

			prompt = misc_classes.Text(f"\"Welcome, my friend, to the Mirror!\" And with that, {character_creation.c_p.lower()} gives a light laugh and disappears in {character_creation.c_eye_color} wisps.")
			break
		...
		case "2":
			...
			prompt = misc_classes.Text(f"You hesitate a moment before squatting down on the ground and trying to nurse your headache. However, the stranger doesn't seem like {character_creation.c_p.lower()} is going to leave, so you wait in silent agony.")
			...
			prompt = misc_classes.Text(f"...And all throughout, the stranger never turns or even moves from {character_creation.c_p_p.lower()} position.")
			...
			prompt = misc_classes.Text(f"You feel your cheeks redden and you internally slap yourself. Of course you'll seem like a wierdo, just watching and waiting. Also stupid. Also - wait, how does {character_creation.c_p.lower()} know that you were even there?")
			character_creation.companion_hearts_base - 10
			prompt = misc_classes.Text(f"\"Rude and dumb too, I suppose,\" the stranger comments dryly. {character_creation.c_p} sighs and stretches {character_creation.c_p_p.lower()} arms above {character_creation.c_p_p.lower()} head.")
			...
			prompt = misc_classes.Text(f"\"That's clear enough,\" the person scoffs, turning away. {character_creation.c_p_p} {character_creation.c_eye_color} orbs narrows as they pass over your form again.")
			...
			prompt = misc_classes.Text(f"The stranger waves {character_creation.c_p_p.lower()} hand dismissively. \"I suppose don't worry about it.\" {character_creation.c_p} stands up, rearranging {character_creation.c_p_p.lower()} cuffs and beginning to walk away.")
			footsteps = misc_classes.Sound_effect("footsteps", 2, "footsteps_channel", -1, 5000, 0)
			prompt = misc_classes.Text(f"You realize that this person probably knows where you are, and maybe - well, should you get {character_creation.c_p_p.lower()} name too? \"H-hey, before you go, where am I?\" you ask, turning around to look at the person.")
			...
			prompt = misc_classes.Text(f"\"Very well, stranger. You're in the world where absolutely nothing makes sense, and the more you think of it, the less your poor brain knows,\" {character_creation.c_p.lower()} continues.")

			cinematic_swell = misc_classes.Sound_effect("cinematic_swell", 7, "cinematic_swell_channel", 0, 0, 0)

			prompt = misc_classes.Text(f"\"Welcome, traveler, to the Mirror.\" And with that, the person gives a slight smirk and disappears in a {character_creation.c_eye_color} light.")
			break
		...
		case "3":
			...
			prompt = misc_classes.Text(f"You blink before walking to the side of the river and ignoring the stranger completely. You walk upstream of {character_creation.c_a_p.lower()}, crouching down by the rushing water.")
			...
			prompt = misc_classes.Text(f"You don't notice it, but the stranger has been watching you for the past minute and {character_creation.c_p_p.lower()} foot has ceased to swing.")
			prompt = misc_classes.Text(f"\"Well, you don't look too good,\" {character_creation.c_p.lower()} murmurs after another moment passes, sliding off of the boulder and walking over to you. \"Tell me - do you need help?\" Looking up, you see {character_creation.c_eye_color} eyes gaze down at you, a bit of sympathy nestled deep within.")
			...
			prompt = misc_classes.Text(f"{character_creation.c_p} lowers {character_creation.c_s_p.lower()} to your side, pulling out a leather pouch from {character_creation.c_p_p.lower()} side. {character_creation.c_p} unlaces the cover and pulls out a white, clean bandage while you continue to drink.")
			prompt = misc_classes.Text(f"The stranger hums quietly as {character_creation.c_p.lower()} dips the cloth into the water and squeezes it of its excess water, gesturing towards your head.")
			prompt = misc_classes.Text(f"\"Allow me,\" {character_creation.c_p.lower()} says, and you glance at {character_creation.c_a_p.lower()} for a gauging moment before nodding slowly.")
			prompt = misc_classes.Text(f"The person gently mops up the blood from your wound, washing it out once more(downstream of where you were drinking) and then wrapping it around your crown. \"What befell you, traveler?\" {character_creation.c_p.lower()} questions, looking at you curiously.")
			...
			prompt = misc_classes.Text(f"You shrug again, taking your eyes off of the stranger and touching your head. \"I don't know. I don't even know where I am.\" You pause, thinking. But before you can say anything, the stranger speaks up again, an amused ring in {character_creation.c_p_p.lower()} voice.")
			prompt = misc_classes.Text(f"\"In that case, traveler, then I welcome you to the world of confusion and puzzles. A world where you're constantly in a race through a labyrinth, running for your life and sanity.\" The stranger smiles slightly at you, {character_creation.c_eye_color} orbs peering at you kindly.")

			cinematic_swell = misc_classes.Sound_effect("cinematic_swell", 7, "cinematic_swell_channel", 0, 0, 0)

			prompt = misc_classes.Text(f"\"Welcome, friend, to the Mirror.\" And with that, the stranger bursts into {character_creation.c_eye_color} flames, disappearing completely.")
			break
